...
public class ShopSettingLoader<R extends ResourceLocation, T extends ShopSetting> extends SimplePreparableReloadListener<Map<R, T>> {
    ...
    public Map<R, T> listResources(ResourceManager pResourceManager, ProfilerFiller pProfiler) {
        ...
        for (Map.Entry<ResourceLocation, Resource> resource : pResourceManager.listResources(FOLDER, p -> p.getPath().endsWith(".json")).entrySet()) {
            try (Reader reader = resource.getValue().openAsReader()) {
                ...
                ShopSetting.CODEC.parse(JsonOps.INSTANCE, element)
                        .resultOrPartial(error -> SeasonShop.logger().debug("Failed to parse ShopSetting data {}", error))
